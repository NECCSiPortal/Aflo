...
function run_tests {
  ...
  if [ setup.cfg -nt aflo.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='aflo/*' --omit='aflo/openstack/common/*' -d covhtml -i
  fi
