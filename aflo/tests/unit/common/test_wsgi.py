...
class RequestTest(test_utils.BaseTestCase):
    ...
    def _set_expected_languages(self, all_locales=None, avail_locales=None):
        ...
        def fake_gettext_find(lang_id, *args, **kwargs):
            found_ret = '/aflo/%s/LC_MESSAGES/aflo.mo' % lang_id
...
class GetSocketTestCase(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
            "aflo.common.wsgi.get_bind_addr",
            lambda x: ('127.0.0.1', 1234)))
        addr_info_list = [(2, 1, 6, '', ('127.0.0.1', 80)),
                          (2, 2, 17, '', ('127.0.0.1', 80)),
                          (2, 3, 0, '', ('127.0.0.1', 80))]
        self.useFixture(fixtures.MonkeyPatch(
            "aflo.common.wsgi.socket.getaddrinfo",
            lambda *x: addr_info_list))
        self.useFixture(fixtures.MonkeyPatch(
            "aflo.common.wsgi.time.time",
            mock.Mock(side_effect=[0, 1, 5, 10, 20, 35])))
        self.useFixture(fixtures.MonkeyPatch(
            "aflo.common.wsgi.utils.validate_key_cert",
            lambda *x: None))
    ...
    def test_correct_configure_socket(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
            'aflo.common.wsgi.ssl.wrap_socket',
            mock_socket))
        self.useFixture(fixtures.MonkeyPatch(
            'aflo.common.wsgi.eventlet.listen',
