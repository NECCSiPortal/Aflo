...
class BaseTestCase(testtools.TestCase):
    ...
    def _copy_data_file(self, file_name, dst_dir):
        src_file_name = os.path.join('aflo/tests/etc', file_name)
...
def fork_exec(cmd,
    ...
    example: aflo-api.  When aflo-api starts it sets itself as
...
class FakeAuthMiddleware(wsgi.Middleware):
    ...
    def process_request(self, req):
        ...
        user = None
        tenant = None
        user_name = req.headers.get('X-User-Name', None)
        tenant_name = req.headers.get('X-Tenant-Name', None)
        roles = []
        if auth_token:
