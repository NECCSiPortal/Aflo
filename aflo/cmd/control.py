...
"""
Helper script for starting/stopping/reloading Aflo server programs.
...
ALL_COMMANDS = ['start', 'status', 'stop', 'shutdown', 'restart',
                'reload', 'force-reload']
ALL_SERVERS = ['api', 'rpcapi']
RELOAD_SERVERS = ['aflo-api', 'aflo-rpcapi']
GRACEFUL_SHUTDOWN_SERVERS = ['aflo-api', 'aflo-rpcapi']
...
def pid_files(server, pid_file):
    ...
    if pid_file:
        ...
        if os.path.exists('/var/run/aflo/%s.pid' % server):
            pid_files = ['/var/run/aflo/%s.pid' % server]
...
def add_command_parsers(subparsers):
    ...
    parser.set_defaults(servers=['aflo-' + s for s in ALL_SERVERS])
